...
```
docker run -d --restart=always -v /var/docker/elasticsearch:/usr/share/elasticsearch/data --net=isolated_elk -p 9200:9200  -p 127.0.0.1:9300:9300 --name elasticsearch elasticsearch:5.4.3

docker run -d --restart=always --link elasticsearch --net=isolated_elk -p 5601:5601 --name kibana kibana:5.4

docker run -d --restart=always --link elasticsearch:db -v /var/docker/logstash:/conf --net=isolated_elk -p 25826:25826 --name logstash logstash:2.3.2-1 logstash -f /conf/syslog.conf
```
